  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto" data-testid="digest-body" ref={scrollBodyRef}>
        ...
        {q.isError && (() => {
          // Friendly handling for the common case: user picked a past
          // date that has no snapshot (404).  Without this, the user
          // saw a raw "HTTP 404 /api/lattice/snapshot…{detail:…}" line.
          const msg = (q.error as Error).message ?? ''
          const isMissingSnapshot =
            /HTTP 404/.test(msg) && /snapshot/.test(msg)
          if (isMissingSnapshot && effectiveDate) {
            return (
              <div className="p-3 text-[11px] leading-[1.6]">
                <div className="text-[var(--color-amber,#e5a200)] font-semibold mb-1">
                  ⚠ No lattice snapshot for {effectiveDate}
                </div>
                <div className="text-[var(--color-dim)]">
                  Pick a different date in the top-right picker, or click{' '}
                  <b className="text-[var(--color-green)]">LIVE</b> to use
                  current data. Today's snapshot is generated by the daily
                  cron at 04:00 UTC, or by clicking the{' '}
                  <b className="text-[var(--color-text)]">↻ fresh</b>{' '}
