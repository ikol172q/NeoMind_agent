...
1. The proposed change deploys to **@your_neomind_test_bot first** (a
...
4. Pick a username ending in `bot` (e.g. `your_neomind_test_bot`).
5. Copy the token BotFather hands back — looks like `1234567890:AbCdEf...`.

### 2. Add the token to `.env`

Open `/Users/localuser/Desktop/NeoMind_agent/.env` and add:
...
```
TG_CANARY_BOT_USERNAME=@your_neomind_test_bot
```
