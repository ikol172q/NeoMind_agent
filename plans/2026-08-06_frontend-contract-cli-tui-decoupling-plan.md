...
**Date:** 2026-08-06  
**Status:** PHASE 4 DONE (2026-08-16) — the REPL turn runs on `AgentSession` by default. **PHASE 5 (Telegram) IN PROGRESS** — the session path is built and wired behind `NEOMIND_TELEGRAM=session`; the default stays `legacy` until a real Telethon round-trip and a container restart sign it off. §11 is the
