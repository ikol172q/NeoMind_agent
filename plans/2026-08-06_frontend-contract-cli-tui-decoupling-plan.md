...
| 7 — be drivable by an existing harness | **DSH drives NeoMind (working 2026-08-17)**; pi blocked upstream | **Scope changed by operator decision.** Building a TUI of our own is deferred, possibly permanently: "自己的 TUI 带来的边际效益其实有待商榷". Both adapters were built instead, and Python was moved to 3.14 first because the ACP SDK requires ≥3.10.
...
**Both clients were then actually run. DSH works; pi does not — and the first three attempts at DSH were wrong in ways worth recording.**

*DSH*: **working.** `dsh --patch` with the config in `tools/protocol/dsh_neomind_subagent.yml` spawns NeoMind and returns its answer — proven by a marker file only NeoMind's start writes, since the reply text is not evidence. Three separate mistakes stood in the way, none of which named itself: npm's `latest` tag for `dsh-subagent-acp` points at `0.0.1-rc.1` while `next` is `0.1.0-rc.7`, so installing without a version silently gets an old release whose "declares no dsh.bundle" warning is a red herring (`dsh-llm` has no `dsh` field either and works); registering the provider is only half, because the model reaches it through a *tool row* that shipped presets carry disabled, and without it the model falls back to the in-process subagent and **reports a delegation that never happened**; and `maxDepth: 'provider-managed'` is required or the plugin tree refuses to load.

Then one bug of ours, visible only through a real client: `stop_reason_for` mapped every non-cancellation `TurnFailed` to ACP's `refusal`, which means *the agent declined*. DSH read it literally, reported "subagent declined the task", and had its model rephrase the prompt twice — a transport failure dressed as a refusal sends the caller to debug the wrong thing. And DSH scrubs credential-shaped variables from child environments by design, so the key must be passed explicitly through `env:` or NeoMind fails with `Illegal header value b'Bearer '`.

*pi*: still blocked, and re-checked with the same skepticism after DSH turned out to be misconfiguration rather than an upstream gap. The npm release is `0.84.2`, the same version as the clone, so this is not a stale-tag problem: `pi client --connect unix:///path` exists in source, but `experimentalCli` — the command tree registering it — has no caller outside its own unit test, so the option is rejected as unknown and `PI_EXPERIMENTAL=1` changes nothing. A Unix socket transport was added because pi's `--connect` accepts no other scheme.

*DSH*: installs and runs (`v0.1.0-rc.7`), but `dsh plugin add @deepseek-ai/dsh-subagent-acp` prints *"declares no dsh.bundle — installed as a plain dependency, not a profile layer"*. The package is `0.0.1-rc.1` with no `dsh` field: it installs and never activates. The `--patch` config shows up in `--dump-config` while the provider is never registered.

**And DSH's model claimed the delegation happened anyway** — "Delegated to the neomind subagent. Its reply: DELEGATION_PROOF" — with no NeoMind process ever spawned, proven by a marker file the configured command would have written. The usual failure in this migration has been a test that lies; this is the system under test lying about itself, which reading its output more carefully makes *more* convincing rather than less. Recorded in `.claude/docs/troubleshooting/20260817-the-system-lied-about-itself.md`.

Our side is finished and verified independently: pi's own client library drives the full session lifecycle, pi's own TypeBox schemas accept 8/8 message shapes, and a real ACP client spawns `acp_stdio.py` over stdio and gets a real answer billed at 118 tokens. `steer` is answered `not_implemented` rather than approximated. Refusals travel in tool content on both protocols, since neither has a refused status — `ToolFinished.denied`, added the same day to stop denials rendering as crashes on the CLI and Telegram, is what makes that reliable in both. |
| dashboard route — audited 2026-08-16 | Not a migration target | **`_handle_dashboard_agent` is a different product, not an unmigrated turn.** Every plain private-DM message goes to it, so it is the surface the operator uses most — which made "migrate it next" look obvious. Reading it says otherwise, and the phase's own task 5 warned about this: *do not silently route special workflows through a generic chat turn*.

What it actually is: loaded through `fin_provider`, so the running implementation lives in the **private fin repo** that provider resolves to, and the copy in this repo is only the open-core baseline. It carries its own `TOOL_SCHEMAS` and `dispatch` rather than `ToolRegistry`, uses the provider's **native** `delta.tool_calls` rather than NeoMind's text protocol, and returns `Proposal` / `AgentReply` — the inline buttons that turn an answer into a recordable investment decision.

Folding it into `AgentSession` would cost the proposal mechanism, push private-repo tools into a public registry, and require reimplementing native tool-call parsing — to gain a tidier diagram. It stays where it is, on the documented compatibility path, and that is the answer rather than a deferral. |
| 8 — compatibility retirement | Task 1 (audit) done, removal deferred by decision (2026-08-16) | **The `/cost` gate, which the phase makes a prerequisite, failed — and not because of the migration.** After a real turn `/cost` reported `$0.0000` and `0 in / 0 out` while the status bar showed thousands. Two independent causes: the pricing lookup read `agent_config._config`, an attribute that stopped existing when the config became a context-scoped proxy, and *both* call sites wrapped it in a bare `except` — so every model priced at zero on the legacy path too, silently, for months. And the session path wrote no usage at all, since it does not go through `QueryEngine`, which owns the budget `/cost` reads. Both fixed; real terminal now shows `$0.0019` / `13212 in / 87 out`. `pricing_unavailable()` added because a broken table and an unpriced model both render `$0.00` and only one is a bug.

**Reverse-dependency audit (task 1).** `QueryEngine` is constructed in `agent/core.py` and **nothing calls its query/run methods** — its only remaining function is holding the `TokenBudget` that `/cost` reads, which is why that budget belonged to an object that never ran a turn. `agent/runtime/` mentions it in four places, all comments, so the import boundary is clean. The other retirement targets still have live callers: `_ask_llm_stream_normal` (11), `_default_llm_call` (23), `_stream_and_render` (16), `_run_agentic_loop` (23).

**Switch alignment.** `NEOMIND_TELEGRAM` defaulted to legacy in code and to session only in `docker-compose.yml`, so the bot ran the migrated path inside its container and the unmigrated one anywhere else, with nothing saying why. All four switches now agree: `legacy` reverts, anything else is the session path, and a typo does not silently revert.

**Removal deferred, deliberately.** The operator's call on `/cost` was "keep what is fixed, invest nothing further", and the same reasoning applies to the removal itself: extracting `TokenBudget` from a 743-line module so `QueryEngine` can be deleted is new construction in service of tidiness, and the dead shell costs nothing while it sits there. Retirement is worth doing when a *second* reason appears — a bug traced to the duplicate paths, or a change that would have to be made twice.

**Recorded, not fixed:** `cost:` in `base.yaml` declares `daily_evolution_budget_usd`, `daily_alert_threshold_usd` and `monthly_budget_cap_usd`, and **no code reads any of them**. Three numbers that look like spending protection and are not. Fleet workers are the only unattended surface, so they are the only place this could matter; wiring it is new work and was not requested. |
