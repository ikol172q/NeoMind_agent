...
**Both clients were then actually run, and neither can drive it yet.**

*pi*: `pi client --connect unix:///path` exists in source, but `experimentalCli` — the command tree registering it — has no caller outside its own unit test, so the option is rejected as unknown and `PI_EXPERIMENTAL=1` changes nothing. A Unix socket transport was added because pi's `--connect` accepts no other scheme.
